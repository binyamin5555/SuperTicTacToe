...
//        SharedPreferences sharedPref = getPreferences(Context.MODE_PRIVATE);
    ...
    private void setComputerPlayers() {
        int location = computerPlayersSpinner.getSelectedItemPosition();
        ...
        computerPlayersSpinner.setSelection(location);   //re-set index
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        ...
        } else if (parent.equals(numOfPlayersSpinner)) {
            int numOfPlayersChosen = position + GameSettings.MIN_NUMBER_OF_PLAYERS;
            editor.putInt(GameSettings.SHARED_PREFS_PLAYERS_NUM_SELECTED, numOfPlayersChosen);
            editor.commit();

            if (numOfPlayersChosen < computerPlayersSpinner.getSelectedItemPosition() + GameSettings.MIN_COMP_PLAYERS) {
                int newNumOfComps = GameSettings.MIN_COMP_PLAYERS;  //reset to 0
                editor.putInt(GameSettings.SHARED_PREFS_COMP_PLAYERS_NUM, newNumOfComps);
                computerPlayersSpinner.setSelection(newNumOfComps); //reposition


            }

            //always
            setComputerPlayers();   //refresh the comp spinner options


        } else if (parent.equals(computerPlayersSpinner)) {
            editor.putInt(GameSettings.SHARED_PREFS_COMP_PLAYERS_NUM, position + GameSettings.MIN_COMP_PLAYERS);
        } else if (parent.equals(magicButtonChanceSpinner)) {
        ...
        } else if (parent.equals(maxPressLimitSpinner)) {
